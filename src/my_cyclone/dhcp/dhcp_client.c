...
void dhcpStateSelecting(DhcpClientCtx *context)
{
   ...
   while(1)
   {
      ...
      if (counter++ == 2)
      {
    	   ...
    	   {
    		   if (context->interface->autoIpContext->running == FALSE)
    		   {
    			   autoIpStart(context->interface->autoIpContext);
    		   }
    	   }
      }
      if ( (osGetSystemTime()-(five_min)) > (60000+1000*60*5))
      {
    		  timeout = DHCP_DISCOVER_INIT_TIMEOUT;
    		  five_min=osGetSystemTime();
      }

      else if ((osGetSystemTime()-(five_min)) > 60000)
      {
    	  timeout = 1000*60*5;
      }
      else
...
error_t dhcpParseAckNak(DhcpClientCtx *context, size_t length)
{
   ...
   //Hack to turn-off AUTO IP and
#if (AUTO_IP_SUPPORT == ENABLED)
   autoIpStop(context->interface->autoIpContext);
   TRACE_INFO("\r\nDisabling Auto-Ip\r\n");
//
//   context->interface->autoIpContext->state=AUTO_IP_STATE_CONFIGURED;
//   osReleaseMutex(&context->interface->autoIpContext->mutex);
#endif
