...
error_t InitOutputs(const char * data, jsmntok_t *jSMNtokens, executor_t ** pCurrentExecutor, jsmnerr_t * resultCode, uint8_t * pos)
{
   ...
   for (i=0;i<MAX_NUM_OUTPUTS;i++)
   {
       ...
       */
      sprintf(&path[0],"/executors/outputs/digital/\\[%i\\]/serial",i);
      ...
      sprintf(&path[0],"/executors/outputs/digital/\\[%i\\]/name",i);
      tokNum = jsmn_get_value(data, jSMNtokens, *resultCode, &path[0]);
      if(tokNum>0)
      {
         length = jSMNtokens[tokNum].end - jSMNtokens[tokNum].start;
   ...
   for (i=0;i<MAX_NUM_OUTPUTS;i++)
   {
       ...
       */
      sprintf(&path[0],"/executors/outputs/analog/\\[%i\\]/serial",i);
      tokNum = jsmn_get_value(data, jSMNtokens, *resultCode, &path[0]);
      if(tokNum>0)
      {
         length = jSMNtokens[tokNum].end - jSMNtokens[tokNum].start;
      ...
      sprintf(&path[0],"/executors/outputs/analog/\\[%i\\]/name",i);
      tokNum = jsmn_get_value(data, jSMNtokens, *resultCode, &path[0]);
      if(tokNum>0)
      {
         length = jSMNtokens[tokNum].end - jSMNtokens[tokNum].start;
      ...
      sprintf(&path[0],"/executors/outputs/analog/\\[%i\\]/place",i);
      tokNum = jsmn_get_value(data, jSMNtokens, *resultCode, &path[0]);
      if(tokNum>0)
      {
         length = jSMNtokens[tokNum].end - jSMNtokens[tokNum].start;
   ...
   pOutputTask = osCreateTask("inputs", outputTask, NULL, configMINIMAL_STACK_SIZE, 3);
