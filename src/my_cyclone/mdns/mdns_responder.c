...
error_t dnsSdSendProbe(NetInterface *interface)
{
	error_t error;
	size_t length;
	size_t offset;
	...
	char *p;
	uint_t i;
	for (i=0;i<interface->numServices;i++)
	{
		...
		p = stpncpy(name,interface->instanceName,DNS_SD_MAX_INSTANCE_LEN);
		*p++ = '.';
		strcpy(p,interface->services[i].name);

		//Length of the mDNS query message
		length = sizeof(DnsHeader);
		//Encode the host name using the DNS name notation
		length += mdnsEncodeName(name, "",
		...
		dnsSdAddSrvResourceRecord(interface, &interface->services[i], message, &length);
		dnsSdAddTxtResourceRecord(interface, &interface->services[i], message, &length);

		//The mDNS query contains one question
		message->qdcount = HTONS(1);
		message->ancount = 0;
		message->nscount = HTONS(2);
		message->arcount = 0;
...
error_t dnsSdSendAnnouncement(NetInterface *interface)
{
	error_t error;
	size_t length;
	size_t offset;
	...
	do
	{
		for (i=0;i<interface->numServices;i++)
		{
			...
			//			{
			//Add PTR record
			error = dnsSdAddPtrResourceRecord(interface, &interface->services[i], FALSE,  message, &length);
			if(error)
			{
				break;
			}
			...
			//			}
			//Add SRV record
			error = dnsSdAddSrvResourceRecord(interface, &interface->services[i], message, &length);
			//Any error to report?
			if(error)
			{
				break;
			...
			//Add TXT record
			error = dnsSdAddTxtResourceRecord(interface, &interface->services[i], message, &length);
			//Any error to report?
			if(error)
			{
				break;
		...
		message->qdcount = 0;
		message->nscount = 0;
		message->arcount = htons(message->arcount);
...
void mdnsTick(NetInterface *interface)
{
	...
	else if(context->state == MDNS_STATE_ANNOUNCING)
	{
		...
		if((time - context->timestamp) >= context->timeout)
		{
			...
			//			dnsSdSendAnnouncement(interface);
			//Save the time at which the packet was sent
			context->timestamp = time;
			...
			{
				//				context->timeout = DNS_SD_DEFAULT_RESOURCE_RECORD_TTL*1000 + netGetRandRange(0, MDNS_INIT_DELAY);
				//				context->state = MDNS_STATE_PROBING;
...
error_t mdnsSendProbe(NetInterface *interface)
{
	...
	//Update the length of the mDNS query message
	length += sizeof(DnsQuestion);
	for (uint i =0; i<interface->numServices; i++)
	{
		dnsSdAddSrvResourceRecord(interface, &interface->services[i], message, &length);
		if (message->ancount)
		{
	...
	message->qdcount = htons(message->qdcount);
	message->ancount = htons(message->ancount);
...
error_t mdnsSendAnnouncement(NetInterface *interface)
{
	...
	do
	{
		...
		for (uint i =0; i<interface->numServices; i++)
		{
			dnsSdAddSrvResourceRecord(interface, &interface->services[i], message, &length);
			dnsSdAddTxtResourceRecord(interface, &interface->services[i], message, &length);
			dnsSdAddPtrResourceRecord(interface, &interface->services[i],FALSE, message, &length);
