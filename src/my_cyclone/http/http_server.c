...
/**
 * @brief Days and months names consts
 **/
...
error_t httpWriteHeader(HttpConnection *connection)
{
   ...
   if ((!connection->response.noCache) && (connection->request.modifiedSince == connection->response.modifiedSince) && (connection->response.statusCode != 404))
   {
   ...
   if (connection->response.useGzipEncoding == TRUE)
   {
   ...
   if (connection->response.statusCode != 304)
   {
...
error_t httpSendResponse(HttpConnection *connection, const char_t *uri)
{
   ...
   FsFile *file;
#if (HTTP_SERVER_GZIPED_FILES == ENABLED)
   if (connection->request.acceptEncodingGzip == TRUE)
   {
   //Retrieve the full pathname
   httpGetGzipedAbsolutePath(connection, uri,
      connection->buffer, HTTP_SERVER_BUFFER_SIZE);

   //Retrieve the date of last modification of file
   error = fsGetFileModDate(connection->buffer, &lastModified);
   if (!error)
       {
   ...
   else
   {
      ...
      connection->response.useGzipEncoding = TRUE;
   }
#endif
#else
   error_t error;
...
#if (HTTP_SERVER_CACHE_CONTROL == ENABLED)
   connection->response.modifiedSince = lastModified;
#endif
