...
#if (HTTP_SERVER_CACHE_CONTROL == ENABLED)
            //If-Modified-Since field found?
            else if(!strcasecmp(name, "If-Modified-Since"))
            {
               //Parse If-Modified-Since field
               httpParseModifiedField(connection, value);
            }
#endif
#if (HTTP_SERVER_GZIPED_FILES == ENABLED)
            //Accept-Encoding field found?
...
void httpParseContentTypeField(HttpConnection *connection, char_t *value)
{
   ...
   if(strcasecmp(value, "application"))
   {

      connection->request.contentType = mimeGetTypeFromType(value);
...
#if (HTTP_SERVER_GZIPED_FILES == ENABLED)
/**
 * @brief Parse Accept-Encoding field
 * @param[in] connection Structure representing an HTTP connection
 * @param[in] value modivied field value
 **/
...
void httpParseAcceptEncodingField(HttpConnection *connection, char_t *value)
{
   ...
   if(token == NULL) return;
      ...
      if (strstr(&token[0],"gzip") != 0)
      {
         if (strstr(&token[0],"deflate") != 0)
         {
            connection->request.acceptEncodingGzip = TRUE;
            return;
         }
      }

      connection->request.acceptEncodingGzip = FALSE;
}
#endif

void httpParseModifiedField(HttpConnection *connection, char_t *value)
{
   char_t *p;
   char_t *token;
   DateTime date;
   ...
   date.day = atoi(&token[5]);//Mon, 22 Dec 2014 22:13:34
   date.year = atoi(&token[12]);
   date.hours = atoi(&token[17]);
   date.minutes = atoi(&token[20]);
   date.seconds = atoi(&token[23]);
   for (date.dayOfWeek = 1; date.dayOfWeek<=7; date.dayOfWeek++)
   {
      if (strstr(&token[0],wday_names[date.dayOfWeek]) != 0)
      {
         break;
      }
   }
   ...
   {
      if (strstr(&token[8],mon_names[date.month]) != 0)
      {
         break;
      }
   }
   connection->request.modifiedSince=convertDateToUnixTime(&date);
}
/**
...
void httpGetGzipedAbsolutePath(HttpConnection *connection,
   const char_t *relative, char_t *absolute, size_t maxLen)
{
   //Copy the root directory
   strcpy(absolute, connection->settings->gzipedDirectory);

   //Append the specified path
   pathCombine(absolute, relative, maxLen);

   //Append the specified path
   pathAddExtention(absolute, ".gz\0", maxLen);
