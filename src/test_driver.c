...
static size_t test_write(Peripheral_Descriptor_t const pxPeripheral, const void * pvBuffer, const size_t xBytes)
{
   ...
   return snprintf(&output[0], 128, "%s#%"PRIu32"!", string, peripheral->peripheralNum);
...
static size_t test_ioctl(Peripheral_Descriptor_t const pxPeripheral, char* pcParameter, char *pcValue)
{
   ...
   if (strncmp(pcParameter, "set_bufer", lenParameter) == 0)
   {
      strncpy(&bufer[0], pcValue, 128);
   }
   lenValue = strlen(bufer);
   return lenValue;
}

register_driver(test_driver,"/test", test_write, test_read, test_ioctl, 12);
register_driver(check_driver,"/check", NULL, test_read1, test_ioctl, 12);
...
void driverTask (void *pvParameters)
{
   ...
   readed = driver_read(fp, &input, 128 );
   driver_close(fp);
